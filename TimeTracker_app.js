    ...
    try {
        ...
        if (!Array.isArray(data)) {
            if (data[selectedDate]) {
                ...
                alert(`TimeTracker_data.json에 [${selectedDate}] 날짜의 데이터가 없습니다.`);
                return;
            }
        } else {
            loadedTasks = data;
        ...
        saveTasks();
        renderTasks();
        alert(`TimeTracker_data.json에서 ${selectedDate} 목록을 성공적으로 불러왔습니다!`);
    } catch (e) {
        console.error(e);
        alert('TimeTracker_data.json 파일을 불러오지 못했습니다. 파일이 존재하는지 확인해주세요.');
    }
}
